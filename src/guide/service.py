...
async def update(
    ...
    try:
        ...
        if not obj:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Note not found"
            )
        if obj.author != author_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="You are unauthorized to perform this task.",
            )
...
async def delete_gui(
    guide_id: str, author_id: str, session: Session = Depends(get_session)
) -> None:
    try:
